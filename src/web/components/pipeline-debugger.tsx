...
export function PipelineDebugger({ job, model_run }: { job: Job; model_run?: ModelRun }) {
  ...
  const start = async (mode: DebugRunMode, stage_id?: string) => {
    ...
    try {
      await runPipelineDebug({ job_id: job.job_id, pipeline_id: active_pipeline_id, mode, stage_id })
    } catch (error) {
          ...
          >
            ...
            <div className="pipeline-debug-runbar">
              ...
              >
                ...
                {snapshot ? "Rerun whole pipeline" : "Run whole pipeline"}
              </button>
            </div>
