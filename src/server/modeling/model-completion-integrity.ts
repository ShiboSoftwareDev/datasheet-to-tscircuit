...
import { parseFreshModelContract, parseModelContract } from "./parse-model-contract"
...
export interface ModelCompletionIntegrityInput {
  ...
  plan: unknown
  result: unknown
  /** Selects the contract policy explicitly; callers may never default to compatibility behavior. */
  policy: ModelCompletionIntegrityPolicy
}

export type ModelCompletionIntegrityPolicy = "fresh_time_voltage_v1" | "legacy_compatibility"
...
function parseHashes(value: unknown, path = "validation-results.json.hashes"): ValidationInputHashes {
  const hashes = record(value, path)
  exactKeys(hashes, ["plan_sha256", "model_sha256", "manifest_sha256"], path)
  const parsed: ValidationInputHashes = {
    plan_sha256: nonEmptyString(hashes.plan_sha256, `${path}.plan_sha256`),
    model_sha256: nonEmptyString(hashes.model_sha256, `${path}.model_sha256`),
    manifest_sha256: nonEmptyString(hashes.manifest_sha256, `${path}.manifest_sha256`),
  }
  for (const [name, hash] of Object.entries(parsed)) {
    if (!SHA256_PATTERN.test(hash)) throw new Error(`${path}.${name} is not SHA-256`)
  }
  return parsed
}
...
function parsePassedResult(
  ...
  exactKeys(
    result,
    ["version", "passed", "hashes", "cases", "errors", "stimulus_causality"],
  ...
  const expected_causality_cases = plan.cases.filter((validation_case) =>
    validation_case.observations.some((observation) => bound_requirement_ids.has(observation.requirement_id)),
  )
  ...
  if (expected_causality_observations > 0) {
    const receipt_path = "validation-results.json.stimulus_causality"
    const receipt = record(result.stimulus_causality, receipt_path)
    exactKeys(
      receipt,
      ["version", "method", "status", "hashes", "checked_case_count", "checked_observation_count"],
      receipt_path,
    )
    if (receipt.version !== 1) throw new Error(`${receipt_path}.version must be 1`)
    if (receipt.method !== "bound_pulse_flatten_v2") {
      throw new Error(`${receipt_path}.method is unsupported`)
    }
    if (receipt.status !== "passed") throw new Error(`${receipt_path}.status must be passed`)
    const receipt_hashes = parseHashes(receipt.hashes, `${receipt_path}.hashes`)
    if (JSON.stringify(receipt_hashes) !== JSON.stringify(hashes)) {
      throw new Error(`${receipt_path}.hashes do not match the validated model inputs`)
    }
    const checked_case_count = finiteNumber(receipt.checked_case_count, `${receipt_path}.checked_case_count`)
    const checked_observation_count = finiteNumber(
      receipt.checked_observation_count,
      `${receipt_path}.checked_observation_count`,
    )
    if (!Number.isInteger(checked_case_count) || checked_case_count !== expected_causality_cases.length) {
      throw new Error(`${receipt_path}.checked_case_count does not cover every bound case`)
    }
    if (
      !Number.isInteger(checked_observation_count) ||
      checked_observation_count !== expected_causality_observations
    ) {
      throw new Error(`${receipt_path}.checked_observation_count does not cover every bound observation`)
    }
  } else if (result.stimulus_causality !== undefined) {
    throw new Error("validation-results.json.stimulus_causality is unexpected for an unbound contract")
  }
...
export function validateModelCompletionIntegrity(
  ...
  try {
    if (input.policy !== "fresh_time_voltage_v1" && input.policy !== "legacy_compatibility") {
      throw new Error(
        "model completion integrity policy must be fresh_time_voltage_v1 or legacy_compatibility",
      )
    }
    if (
    ...
    const contract =
      input.policy === "fresh_time_voltage_v1"
        ? parseFreshModelContract(input.contract)
        : parseModelContract(input.contract)
    ...
    const result = parsePassedResult(input.result, plan, contract)
