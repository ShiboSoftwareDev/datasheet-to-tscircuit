...
test("model previews read persisted Circuit JSON and never rerun TSX on selection", async () => {
  ...
  expect(model_run?.reference_preview?.x_axis_label).toBe("Time")
  expect(model_run?.reference_preview?.x_axis_unit).toBe("ms")
  expect(model_run?.reference_preview?.y_axis_label).toBe("Voltage")
  expect(model_run?.reference_preview?.y_axis_unit).toBe("V")
